...
ax[0, 0].text(0.5, 0.9, r'$\frac{\lambda_1+ \lambda_2 \xi_T}{2} \leq -wX_0$', fontsize=20,
...
ax[0, 1].text(0.5, 0.9, r'$-wX_0 \leq \frac{\lambda_1+ \lambda_2 \xi_T}{2} \leq 0$', fontsize=20,
...
ax[0, 2].text(0.5, 0.9, r'$0 \leq \frac{\lambda_1+ \lambda_2 \xi_T}{2}$', fontsize=20,
...
ax[1, 0].text(0.5, 0.9, r'$\frac{\lambda_1+ \lambda_2 \xi_T}{2} \leq -wX_0$', fontsize=20,
              transform=ax[1, 0].transAxes, horizontalalignment='center', verticalalignment='top')
ax[1, 0].set_ylabel('$\lambda_2<0$', labelpad=50, size=20, rotation='horizontal', verticalalignment='center')
...
ax[1, 1].text(0.5, 0.9, r'$-wX_0 \leq \frac{\lambda_1+ \lambda_2 \xi_T}{2} \leq 0$', fontsize=20,
...
ax[1, 2].text(0.5, 0.9, r'$0 \leq \frac{\lambda_1+ \lambda_2 \xi_T}{2}$', fontsize=20,
...
#         points = np.array([[z, case1.mv(z)[0]] for z in np.linspace(source.z0 - 0.5, source.z0 + 0.5)
#                            if case1.mv(z)[1]]).T.reshape(2, -1)
...
for i in enumerate([0.3, 0.4, 0.8]):
    for j in enumerate([0.1, 0.5, 1]):
        source.attribution(w=i[1], sigma=j[1])
        points = np.array([[z, MontelCarlo.mv(z)[0]] for z in np.linspace(source.z0 - 0.5, source.z0 + 0.5)
                           if MontelCarlo.mv(z)[1]]).T.reshape(2, -1)
        ax[i[0], j[0]].plot(points[0], points[1])
        ax[i[0], j[0]].axvline(ymax=0.8, x=source.z0, c='C1', linestyle='dashed')
        ax[i[0], j[0]].text(0.5, 0.95, r'$w={}, \sigma={}$'.format(i[1], j[1]), fontsize=20,
                            transform=ax[i[0], j[0]].transAxes, horizontalalignment='center', verticalalignment='top')
...
#         source.attribution(w=i[1], sigma=j[1])
#         points = np.array([[z, case3.mv(z)[0]] for z in np.linspace(source.z0 - 0.5, source.z0 + 0.5)
#                            if case3.mv(z)[1]]).T.reshape(2, -1)
#         ax[i[0], j[0]].plot(points[0], points[1])
#         ax[i[0], j[0]].axvline(ymax=0.8, x=source.z0, c='C1', linestyle='dashed')
#         ax[i[0], j[0]].text(0.5, 0.95, r'$w={}, \sigma={}$'.format(i[1], j[1]), fontsize=20,
#                             transform=ax[i[0], j[0]].transAxes, horizontalalignment='center', verticalalignment='top')
...
#     ax[i, 0].set_ylabel('variance', fontsize=20)
#     ax[2, i].set_xlabel('mean', fontsize=20)
# fig.savefig('figures/case3/sigma_w.png')


# # Painting w = 1, b = 0, which is the case of Bielecki(2005)
# source.attribution(w=1, b=0)
# fig, ax = plt.subplots(3, 3, figsize=(16, 10))
# for i in enumerate([0.1, 1, 10]):
#     for j in enumerate([0.1, 0.5, 1]):
#         source.attribution(x0=i[1], sigma=j[1])
#         points = np.array([[z, case1.mv(z)[0]] for z in np.linspace(source.z0 - 0.5, source.z0 + 0.5)
#                            if case1.mv(z)[1]]).T.reshape(2, -1)
#         ax[i[0], j[0]].plot(points[0], points[1])
#         ax[i[0], j[0]].axvline(ymax=0.8, x=source.z0, c='C1', linestyle='dashed')
#         ax[i[0], j[0]].text(0.5, 0.95, r'$x_0={}, \sigma={}$'.format(i[1], j[1]), fontsize=20,
#                             transform=ax[i[0], j[0]].transAxes, horizontalalignment='center', verticalalignment='top')
# for i in range(3):
#     ax[i, 0].set_ylabel('variance', fontsize=20)
#     ax[2, i].set_xlabel('mean', fontsize=20)
# fig.savefig('figures/zhou_x0_sigma.png')
